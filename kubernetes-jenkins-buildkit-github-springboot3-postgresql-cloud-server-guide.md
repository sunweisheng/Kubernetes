...
如果还不熟悉 Chart、Values、Template 和 Manifest 的关系，先阅读 [Helm 与 Kubernetes 交互原理及使用指南](./helm-kubernetes-interaction-guide.md)。如果需要理解同一个项目如何在虚拟机和云服务器中使用同一份 Chart、只替换环境差异并生成不同的最终 Ingress 和 Deployment 配置，请阅读 [同一项目的多环境 Helm Values 与构建结果指南](./helm-multi-environment-values-guide.md)。该附件讲解 Helm 3 默认的 Secret 发布记录；本方案为了缩小 Jenkins 部署账号权限，显式使用 `HELM_DRIVER=configmap` 保存 Spring Boot Release 记录。
...
这里的 `deploy-overrides-values.yaml` 是“环境差异文件”，不是完整的 Chart 配置，也不是把项目仓库里的 `values.yaml` 改掉。它只写云服务器和虚拟机不同的两个字段：
...
这里的“替换默认值”不是替换服务器上的原文件，而是 Helm 在本次 `lint`、`template` 和 `upgrade` 渲染时，按同一条配置路径覆盖内存中的值：

- `ingress.host` 是配置路径，先找到 `ingress` 对象，再替换其中的 `host` 字段。
- `ingress.tlsSecret` 同理，只替换 TLS Secret 名称。
- Chart 的 Ingress 模板必须读取 `.Values.ingress.host` 和 `.Values.ingress.tlsSecret`，这些键名才能产生作用；覆盖文件不是任意写一个同义字段就会自动生效。
- 覆盖文件没有写 `ingress.enabled`、`ingress.ingressClassName`、路径等字段，这些字段继续使用 Chart 默认值。
- 如果只写 `ingress.host`，只改域名；如果只写 `ingress.tlsSecret`，只改 Secret 名称；没有写的字段不会被清空。
- 字段名和 YAML 缩进必须正确。写成 `hostName`、`tls.secretName` 或把 `host` 放到 `ingress` 外面，都不会覆盖 Chart 当前使用的 `ingress.host`。

这三个“文件/对象”要分开理解：

1. `deploy-overrides-values.yaml` 是 `hk-k8s-master` 上的普通文件，供创建 ConfigMap 时读取。
2. `ConfigMap/deploy-overrides` 是 Kubernetes 中的配置对象；`--from-file=values.yaml=...` 把普通文件的内容保存到名为 `values.yaml` 的数据键中。文件名和 ConfigMap 数据键可以不同，这里故意把键名固定为 `values.yaml`，因为 Agent 挂载后要按这个文件名读取。
3. `${HELM_OVERRIDE_VALUES_FILE}` 是临时 Agent Pod 内的文件路径。`ci/prepare-helm-values.sh` 会把挂载的 `values.yaml` 复制到这个路径；如果 ConfigMap 不存在，则生成 `{}`，表示没有环境覆盖。Helm 通过 `valuesFiles` 读取的是 Agent 内的这个路径，不是 master 上的 `deploy-overrides-values.yaml`。

因此，云端的完整链路是：

```text
master 上的 deploy-overrides-values.yaml
  → kubectl 创建 ConfigMap/deploy-overrides
  → 新 Agent Pod 挂载为 values.yaml
  → prepare-helm-values.sh 复制到 HELM_OVERRIDE_VALUES_FILE
  → Helm valuesFiles 读取
  → 覆盖 Chart 默认的域名和 TLS Secret
```
...
`controller.admin.existingSecret: jenkins-admin` 表示 Jenkins 使用第 9.2 节已经创建的管理员 Secret。Jenkins Chart 只有在 `existingSecret` 为空且 `createSecret: true` 时，才会按照 `admin.username` 和 `admin.password` 创建自己的 Secret；当前云服务器配置虽然写了 `createSecret: true`，但因为已经填写 `existingSecret`，不会创建或覆盖 `jenkins-admin`。管理员用户名和密码仍然只来自 `jenkins-admin`，不要写进 values 文件。

JCasC 是 Jenkins 的 `Configuration as Code` 插件和配置方式，不是 Kubernetes 控制器，也不是独立 Pod。`installPlugins` 中的 `configuration-as-code` 插件运行在 Jenkins Controller 内部；Helm Chart 把 `JCasC.defaultConfig` 和 `configScripts` 生成 ConfigMap 并挂载到 Controller，JCasC 插件在 Jenkins 启动时读取这些 YAML，配置 Jenkins 自己的 Kubernetes Cloud、GitHub 设置、Remoting 安全和全局共享类库。它不负责创建 Kubernetes 的 Ingress、Service、Role 或 Pod，那些对象仍由 Helm Chart 和 Kubernetes 处理。

当前云服务器配置中的 `github-api-usage`、`v3-security` 和 `v3-shared-library` 都是 Jenkins 内部配置。`v3-shared-library` 把公开的 `jenkins-json-build` 仓库、`shared-library` 子目录和固定版本 `v3.2.0` 注册为全局共享类库；Jenkinsfile 仍应使用固定标签显式引用它。JCasC 的 `configScripts` 使用 YAML 的 `|` 多行字符串，缩进内容会作为独立的 Jenkins 配置文件提交。
...
Sidecar 是和 Jenkins Controller 在同一个 Pod 中并行运行的辅助容器，不是 initContainer，也不是独立的 Jenkins 服务。Jenkins Chart 的 `sidecars.configAutoReload` Sidecar 会监视 JCasC ConfigMap 变化并请求 Jenkins 重新加载配置。本方案设置 `enabled: false`，所以不创建这个额外容器，不自动实时重载 JCasC；修改后按 Helm 更新或重启流程在下一次启动时加载，也不需要为 Sidecar 增加读取 ConfigMap 的 Role。
...
`readSecrets: false` 只禁止直接查询 Secret API，不等于完全阻止由 Jenkins 创建的 Pod 间接接触 Secret。能够创建 Pod 的身份在某些配置下可能尝试引用 Secret 卷，因此生产环境还应结合独立命名空间、Pod 安全策略、镜像白名单、NetworkPolicy 和受控流水线来源进一步限制。这里的 `jenkins` 是 Controller 创建 Agent Pod 的身份；Agent 的 `jenkins-build` ServiceAccount 不等于 Controller 的 RoleBinding，也不应与第 10 节的 `jenkins-deployer` 混用。
...
一次 Jenkins HTTPS 请求的路径是：培训电脑通过域名和 hosts 找到入口节点的 `30443` NodePort，Traefik 接收 HTTPS 并使用 Ingress 引用的 TLS Secret 完成证书处理；Traefik 再按 `hostName` 和 `/` 的 `Prefix` 规则，把请求转给 `ci` 命名空间的 `jenkins` ClusterIP Service:8080，Service 最后选择 Jenkins Controller Pod。TLS Secret 必须和 Ingress 在同一个命名空间。没有 Traefik、IngressClass 不匹配、TLS Secret 不存在或 Service 没有后端时，Ingress 对象仍可能存在，但访问链路不会成功。
...
这一段不是向公网证书机构购买证书，而是在 `hk-k8s-master` 本地建立一套只供本实验使用的“小型证书机构”。它先创建一张可以签发证书的本地 CA，再用这张 CA 为 Jenkins、Spring Boot 和 Headlamp 的 HTTPS 入口签发一张服务器证书。培训电脑信任 `k8s-cloud-lab-ca.crt` 后，浏览器才会信任由它签发的 `wildcard-k8s-cloud-lab.crt`。
...
CA 私钥 + CA 证书
       │签名
       ▼
服务器私钥 + CSR + 域名配置  ──>  wildcard-k8s-cloud-lab.crt
```

其中，CA 私钥是“签发权”；服务器私钥是 Traefik 解密 HTTPS 流量时使用的私钥；服务器证书是发给访问者看的身份证；CSR 是申请签发服务器证书的中间文件。Kubernetes TLS Secret 最终只需要服务器证书和服务器私钥，不需要 CSR；培训电脑需要信任 CA 证书，不需要把 CA 私钥放进 Kubernetes。
...
- `source` 是 Bash 的内置命令，表示在当前终端读取并执行 `platform.env`。本段后面的 `${JENKINS_HOST}`、`${APP_HOST}` 和 `${HEADLAMP_HOST}` 都依赖这个文件；如果这些变量没有值，证书可能缺少正确的域名。它不是读取 Kubernetes Secret，也不是把配置上传到云平台。
- `TLS_DIR` 只是当前 Shell 中的路径变量，指向证书目录。`$HOME` 代表当前登录用户的家目录；云服务器示例中通常是 `/root`，所以最终目录类似 `/root/k8s-platform/tls`。
- `mkdir -p` 创建目录；目录已经存在时不会报错，所以重试脚本不会因为目录存在而停止。
- `chmod 700` 让目录所有者可以读、写、进入目录，其他用户没有任何权限。目录权限很重要，因为里面会放 CA 私钥和服务器私钥。
- `umask 077` 是本次 Shell 后续创建文件时的默认权限限制。新文件默认只允许当前用户读写，避免私钥刚生成时就对同一台机器上的其他用户开放。它不会改变已经存在文件的权限。
...
- `openssl genrsa ... 4096` 生成 4096 位 RSA 私钥，保存为 `k8s-cloud-lab-ca.key`。这是真正的签发权：任何拿到它的人，都可以为其他域名签发一张看起来由本实验 CA 签发的证书。因此它不能提交 Git、不能发到聊天，也不应放进 Kubernetes Secret。
...
- `-key` 指定刚才生成的 CA 私钥；`-subj` 直接填写主题名称，所以脚本执行时不会进入交互式提问；`-out` 将公开的 CA 证书写入 `k8s-cloud-lab-ca.crt`。
- `k8s-cloud-lab-ca.crt` 是培训电脑需要信任的“根证书”。它的 `CN` 只是这张 CA 的名称，不是 Jenkins 的访问域名；服务器域名会在下一段的 SAN 中写入服务器证书。
...
| `[dn]` 的 `CN` | 设置证书主题的通用名称。这里写 `*.cloud.k8s.lab`，主要用于展示和兼容旧客户端。现代浏览器主要依据 SAN 判断域名。 |
...
| `[alt_names]` | 列出浏览器真正要匹配的域名。通配符 `*.cloud.k8s.lab` 可以覆盖 `jenkins.cloud.k8s.lab`、`app.cloud.k8s.lab` 和 `headlamp.cloud.k8s.lab` 这一层；三个 `DNS.2` 到 `DNS.4` 又把当前实际主机名明确写入证书。 |

通配符只匹配一个左侧标签：它可以匹配 `jenkins.cloud.k8s.lab`，但不能匹配 `api.jenkins.cloud.k8s.lab`，也不能匹配裸域名 `cloud.k8s.lab`。因此域名改动后，必须重新生成包含新域名的证书；不能只修改 `/etc/hosts`。
...
- `-CAcreateserial` 为这次签发创建或使用 CA 序列号文件，通常会留下 `k8s-cloud-lab-ca.srl`。它用于给每张签发证书分配不同的序列号，不是私钥，也不会被 Traefik 读取。
- `-out` 生成最终的 `wildcard-k8s-cloud-lab.crt`。它和同名 `.key` 配对，之后会作为 TLS Secret 的 `tls.crt` 和 `tls.key`。

CA 的有效期约 10 年，服务器证书的有效期约 825 天。服务器证书到期时可以用同一 CA 重新签发；如果 CA 私钥泄露，则不能只换服务器证书，必须重新建立 CA 并重新让培训电脑信任新的 CA。
...
- `openssl verify` 使用 `-CAfile` 指定信任根 CA，然后检查服务器证书是否能由这张 CA 正确验证。输出 `...wildcard-k8s-cloud-lab.crt: OK` 表示签名链和证书基本有效。
...
| 文件 | 用途 | 后续去向 |
| --- | --- | --- |
| `k8s-cloud-lab-ca.key` | 本地 CA 私钥，具有签发权 | 只受保护地备份，不放进 Kubernetes。 |
| `k8s-cloud-lab-ca.crt` | 本地 CA 公钥证书 | 导入培训电脑的信任库，也可作为备份。 |
| `wildcard-k8s-cloud-lab.key` | Traefik 解密 HTTPS 的服务器私钥 | 作为 Kubernetes TLS Secret 的 `tls.key`。 |
| `wildcard-k8s-cloud-lab.csr` | 服务器证书申请文件 | 签发完成后只作排查依据，不上传到 Kubernetes。 |
| `wildcard-k8s-cloud-lab.crt` | 由本地 CA 签发的服务器证书 | 作为 Kubernetes TLS Secret 的 `tls.crt`。 |
| `wildcard-k8s-cloud-lab.cnf`、`k8s-cloud-lab-ca.srl` | 证书扩展配置和序列号记录 | 便于排查或再次签发，不能代替私钥和证书。 |
...
② wildcard-k8s-cloud-lab.cnf + wildcard-k8s-cloud-lab.key
        │生成证书申请
        ▼
   wildcard-k8s-cloud-lab.csr

③ k8s-cloud-lab-ca.key + k8s-cloud-lab-ca.crt
  + wildcard-k8s-cloud-lab.csr + wildcard-k8s-cloud-lab.cnf
        │CA 签名
        ▼
   wildcard-k8s-cloud-lab.crt

④ wildcard-k8s-cloud-lab.crt + wildcard-k8s-cloud-lab.key
...
第 ③ 步中的 `k8s-cloud-lab-ca.key` 和 `k8s-cloud-lab-ca.crt` 一起代表本地 CA。CA 证书中包含 CA 的公钥，CA 私钥则留在签发端。两者共同形成“谁签发了服务器证书”的信任关系，但只有 CA 私钥能签名，CA 证书本身不能签发证书。

`wildcard-k8s-cloud-lab.key` 是服务器私钥。生成 CSR 时，OpenSSL 会把与它对应的服务器公钥和域名申请信息放入 `wildcard-k8s-cloud-lab.csr`，并用服务器私钥证明这份申请确实由私钥持有者发起。CA 使用自己的 `k8s-cloud-lab-ca.key` 对 CSR 签名后，才得到 `wildcard-k8s-cloud-lab.crt`。因此，`wildcard-k8s-cloud-lab.crt` 必须和 `wildcard-k8s-cloud-lab.key` 成对使用；换成另一把私钥，Traefik 会无法完成 TLS 握手。

`wildcard-k8s-cloud-lab.cnf` 不是证书，而是生成证书时使用的“申请表模板”：它告诉 OpenSSL 把哪些域名写进 SAN、证书用于什么场景以及允许哪些密钥用途。`k8s-cloud-lab-ca.srl` 也不是证书，它只是 CA 签发证书时记录序列号的辅助文件。它们不会被 Traefik 读取，也不会放入 TLS Secret；以后需要按相同规则重新签发证书时，`.cnf` 可以继续参考。

真正运行 HTTPS 时，CA 私钥不会参与每一次访问。访问链路是：

1. 浏览器访问 `https://jenkins.cloud.k8s.lab:30443`，并在 TLS 握手中告诉 Traefik 要访问的主机名。
2. Traefik 从当前命名空间的 TLS Secret 读取 `wildcard-k8s-cloud-lab.crt` 和 `wildcard-k8s-cloud-lab.key`，用证书说明自己的身份，并用私钥证明自己确实拥有这张证书对应的私钥。
3. 浏览器检查证书的 SAN 是否包含 `jenkins.cloud.k8s.lab`，再检查签发者是否是自己已经信任的 `k8s-cloud-lab-ca.crt`。
4. 两边完成校验后，浏览器和 Traefik 协商出加密会话；Traefik 再把请求转给 Jenkins Service。

第 2 段命令把同一对 `wildcard-k8s-cloud-lab.crt` 和 `wildcard-k8s-cloud-lab.key` 分别复制到 `ci`、`spring-app` 和 `headlamp` 命名空间，是因为 Kubernetes 的 Ingress 只能引用自己所在命名空间的 Secret。三个 Secret 内容相同，但对象彼此独立；删除其中一个不会删除另外两个。`k8s-cloud-lab-ca.crt` 不需要放入这三个 TLS Secret，它只需要导入培训电脑的信任库。
...
第 1 段生成的是文件；第 2 段才是把服务器证书和服务器私钥登记到 Kubernetes。它不会重新生成证书，也不会把 CA 私钥放进集群，而是把同一对 `wildcard-k8s-cloud-lab.crt` 和 `wildcard-k8s-cloud-lab.key` 分别创建为三个命名空间中的 TLS Secret。
...
- `source` 把 `platform.env` 中的 `CI_NAMESPACE`、`APP_NAMESPACE`、`HEADLAMP_NAMESPACE` 和 `TLS_SECRET_NAME` 读入当前 Shell。当前云方案中它们分别指向 `ci`、`spring-app`、`headlamp` 和同一个 TLS Secret 名称。
- `TLS_DIR` 指向第 1 段生成证书的目录。这里使用变量，是为了避免在后面每条命令中重复写 `/root/k8s-platform/tls`。
- 如果这些变量没有正确加载，`kubectl -n` 可能收到空命名空间，或者 Secret 名称不一致。因此这段命令必须在已经准备好 `platform.env` 的 `hk-k8s-master` 上执行。
...
- `"$TLS_SECRET_NAME"` 是对象名称，例如 `k8s-cloud-lab-tls`。同一个名称可以同时存在于三个不同命名空间，因为名称的唯一范围是“命名空间内”。
- `--cert` 指向服务器证书 `wildcard-k8s-cloud-lab.crt`，写入 Secret 的 `tls.crt` 字段。
- `--key` 指向与证书匹配的服务器私钥 `wildcard-k8s-cloud-lab.key`，写入 Secret 的 `tls.key` 字段。
- CA 证书 `k8s-cloud-lab-ca.crt`、CA 私钥、CSR、`.cnf` 和 `.srl` 都不属于这个 TLS Secret。CA 证书由培训电脑的信任库使用，CA 私钥只用于将来签发证书。
...
data:
  tls.crt: <wildcard-k8s-cloud-lab.crt 的 Base64 内容>
  tls.key: <wildcard-k8s-cloud-lab.key 的 Base64 内容>
```
...
整段命令的实际结果是：同一张服务器证书被三个 Ingress 分别引用，Traefik 可以在 `ci`、`spring-app` 和 `headlamp` 命名空间找到各自的 Secret；培训电脑则通过已经信任的 CA 证书验证这张服务器证书。证书文件本身没有被复制三份到磁盘，复制的是 Kubernetes 中三个命名空间各自保存的 Secret 对象。
...
这里的文件名容易混淆：`deploy-overrides-values.yaml` 是 master 上创建 ConfigMap 时的输入文件；ConfigMap 内部的键名是 `values.yaml`；流水线实际交给 Helm 的是 Agent Pod 内由 `HELM_OVERRIDE_VALUES_FILE` 指向的临时文件。三者内容相同，但所在位置和使用者不同。删除 master 上的普通文件不会自动删除已经创建的 ConfigMap；修改普通文件后也不会自动改变集群，必须重新执行本节的 ConfigMap 更新命令。
